...
{
public:
	UINT	m_iVertex[2];		//from Vertex0 to Vertex1

	UINT	m_iTwinEdge;		//same edge from vertex1 to vertex0
...
{
public:
	UINT		m_nVertex;				//number of vertices
	CVertex*	m_pVertex;				//
...
public:
	...
	bool	Save(const char* sFileName);	// save to file
	  ...
	  friend std::istream& operator >> (std::istream &input, CMesh &mesh);
